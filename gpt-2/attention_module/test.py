...
def generate_matrix(M, N, sparse_ratio):
    ...
    return np.random.choice([0, 1], size=(M, N), p = [sparse_ratio, 1 - sparse_ratio])
...
def test():
    ...
    xwqk = matmul.forward(xwq, x.shape[0], x.shape[1], wq.shape[1], test=True)

    

    if np.allclose(expected_xwqk, xwqk, rtol=1e-2, atol=1e-2):
        print("✓ True!")
    else:
        print("✗ False!")
        print("diff: ")
        print(expected_xwqk - xwqk)
