...
def bf16_to_float_block(bf16_block):
    ...
    for bf16 in bf16_block:
        fp32_bits = bf16 << 16
        float_block.append(struct.unpack(
            '>f', struct.pack('>I', fp32_bits))[0])
    # 转换为浮点数
    return float_block
...
class MACUnit:
    ...
    def tick(self):
        ...
        self.multliply_pipeline.clock_cycle(
...
class AdderTree:
    ...
    def tick_first_stage(self):
        ...
        self.add_pipeline_lower[0].clock_cycle(
            self.values_pack[0][0], self.values_pack[0][1], self.values_pack[0][2])
        self.add_pipeline_lower[1].clock_cycle(
            self.values_pack[1][0], self.values_pack[1][1], self.values_pack[1][2])
    ...
    def tick_second_stage(self):
        ...
        final_result = None
        # 计算第二级加法结果，确保类型一致
        self.add_pipeline_upper.clock_cycle(
            self.intermediate_results[0], self.intermediate_results[1], True)
        ...
        for cycle in self.cycles:
            ...
            if "input" in cycle:
                if cycle["input"] is None:
                    ...
                    print(
                        f"  输入: 矩阵元素 C[{cycle['input']['coords'][0]}][{cycle['input']['coords'][1]}] - 块 {cycle['input']['block_idx']}")

            # 阶段状态
            print(
                f"  阶段1: {'有效' if cycle.get('stage1_valid', False) else '无效'}")
            print(
                f"  阶段2: {'有效' if cycle.get('stage2_valid', False) else '无效'}", end="")
            if cycle.get('stage2_valid', False) and "stage2_coords" in cycle:
                print(
                    f" - 处理 C[{cycle['stage2_coords'][0]}][{cycle['stage2_coords'][1]}]")
            else:
                print("")

            print(
                f"  阶段3: {'有效' if cycle.get('stage3_valid', False) else '无效'}", end="")
            if cycle.get('stage3_valid', False) and "stage3_coords" in cycle:
                print(
                    f" - 处理 C[{cycle['stage3_coords'][0]}][{cycle['stage3_coords'][1]}]")
            else:
                print("")

            print(
                f"  阶段4: {'有效' if cycle.get('stage4_valid', False) else '无效'}", end="")
...
def pipeline_matmul(A, B, verbose=False):
    ...
    # 创建元素跟踪字典和输入队列
    # {(i,j): {"blocks_total": x, "blocks_processed": y, "result": value}}
    element_tracker = {}
        ...
        for j in range(n):
            ...
            for block_start in range(0, k_len, sim.mac_width):
                ...
                if len(a_block) < sim.mac_width:
                    a_block = np.pad(
                        a_block, (0, sim.mac_width - len(a_block))).astype(np.float32)
                    b_block = np.pad(
                        b_block, (0, sim.mac_width - len(b_block))).astype(np.float32)
    ...
    while input_queue or sim.is_active():
        ...
        partial_result = sim.process_block(
            a_block, b_block, coords, is_last_block)
        import struct
