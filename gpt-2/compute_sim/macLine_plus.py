"""MAC单元流水线实现，用于BF16浮点数的矩阵乘法运算模拟。提供了完整的流水线实现，包括乘法单元和加法单元。"""
...
class MatmulPipeline:
    ...
    def clock_cycle(self, a_block, b_block, coords, valid):
        ...
        self.add.get_input(
            self.stage3_result[0], self.stage3_result[1], self.stage3_valid)
        ...
        temp_result = [0] * 2
        self.addvec[0].get_input(
            self.stage2_result[0], self.stage2_result[1], self.stage2_valid)
        self.addvec[1].get_input(
            self.stage2_result[2], self.stage2_result[3], self.stage2_valid)
        ...
        for i, mac in enumerate(self.macline):
            mac.get_input(
                self.stage1_a_block[i], self.stage1_b_block[i], self.stage1_valid)
            mac.clock_cycle()
