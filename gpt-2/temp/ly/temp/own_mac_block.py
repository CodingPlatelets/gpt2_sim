...
class ProcessingElement:
    ...
    def __init__(self, pe_id):
        ...
        self.global_column_map = {}  # 将局部列映射到全局列
...
class DistributionNetwork:
    ...
    def append_matrices(self, a_matrix, b_csc):
        """追加矩阵数据到现有队列，而不是重置"""
        self.a_matrix = a_matrix
    ...
    def has_pending_work(self, global_col):
        """检查是否还有针对特定全局列的待处理工作"""
        # 检查所有队列中是否有该列的待处理数据
        for queue in self.queues:
            for _, _, col_idx in queue:
                if col_idx == global_col:
                    return True
        return False
    ...
    def is_busy(self):
        """检查分发网络是否处于忙碌状态"""
        return self.stage_fetch_a_valid or self.stage_distribute_valid
...
class SparseMatrixPipeline:
    ...
    def clock_cycle(self):
        ...
        if not self.distribution_network.is_busy() and \
            ...
            if self.result is None:
                logger.warning("结果矩阵未初始化，创建新矩阵")
                if hasattr(self, 'A') and hasattr(self, 'B_dense_full'):
                    self.result = np.zeros((self.A.shape[0], self.B_dense_full.shape[1]))
                else:
                    # 创建一个默认大小的结果矩阵
                    self.result = np.zeros((1, 128))
    ...
    def run_simulation(self, max_cycles=10000):
        ...
        while self.clock < max_cycles:
            if self.clock % 100 == 0:  # 减少日志频率，提高效率
                queue_lengths = self.distribution_network.get_queue_lengths()
                busy_pes = sum(1 for pe in self.pes if pe.is_busy())
                logger.info(f"周期 {self.clock}: 最大队列长度={max(queue_lengths) if queue_lengths else 0}, 活跃PE={busy_pes}")
    ...
    def run_continuous_pipeline_matrix_multiplication(self, a_rows=1, a_cols=4096, b_rows=4096, b_cols=4096, 
                        sparsity=0.8, block_size=128, seed=42):
        """
        通过真正的连续流水线方式处理大规模矩阵乘法，避免块间重置
        """
        logger.info(f"开始连续流水线矩阵乘法 ({a_rows}x{a_cols}) * ({b_rows}x{b_cols})")
        ...
        # 计算块数
        num_blocks = (b_cols + block_size - 1) // block_size
        
        # 初始化全局结果矩阵和self.result
        self.result = np.zeros((a_rows, b_cols))
        ...
        logger.info(f"开始连续流水线矩阵乘法模拟")
        
        # 主循环：一次处理一个块，但不重置PE状态
        while processed_blocks < num_blocks:
            # 计算当前块的列范围
            start_col = processed_blocks * block_size
            end_col = min((processed_blocks + 1) * block_size, b_cols)
            actual_block_size = end_col - start_col
            
            logger.info(f"处理块 {processed_blocks+1}/{num_blocks}: 列 {start_col} 到 {end_col-1}")
            
            # 创建这个块的全局列映射
            column_map = {}
            for local_col in range(actual_block_size):
                global_col = start_col + local_col
                column_map[local_col] = global_col
            
            # 提取当前块的B矩阵
            B_block = self.B_dense_full[:, start_col:end_col]
            block_csc = CSCMatrix.from_dense(B_block)
            ...
            # 将当前块加载到分发网络
            self.distribution_network.reset()  # 重置分发网络
            self.distribution_network.load_matrices(self.A, block_csc)
            
            # 每个块单独运行，直到完成
            block_clock = 0
            while self.is_active() and block_clock < 10000:  # 最大允许10000个周期
                self.clock_cycle()
                block_clock += 1
                
                # 减少日志输出频率
                if block_clock % 1000 == 0:
                    queue_lengths = self.distribution_network.get_queue_lengths()
                    busy_pes = sum(1 for pe in self.pes if pe.is_busy())
                    logger.info(f"块 {processed_blocks+1} 周期 {block_clock}: 队列={max(queue_lengths) if queue_lengths else 0}, 活跃PE={busy_pes}")
            
            # 块处理完成后，收集当前块的结果
            for pe_idx, pe in enumerate(self.pes):
                for local_col, value in pe.accumulators.items():
                    if local_col in column_map:  # 确认这是一个有效的映射
                        global_col = column_map[local_col]
                        self.result[0, global_col] = value  # 确保使用=而不是+=
            
            logger.info(f"块 {processed_blocks+1}/{num_blocks} 完成，用时 {block_clock} 个周期")
            processed_blocks += 1
    ...
    def verify_result(self):
        ...
        logger.info(f"连续流水线矩阵乘法完成，总用时 {self.clock} 个周期")
...
def main():
    ...
    # simulator.run_continuous_pipeline_matrix_multiplication(
    #     a_rows=1, a_cols=4096, b_rows=4096, b_cols=4096, 
    #     sparsity=0.9, block_size=PE_SIZE)
    simulator.run_large_matrix_multiplication(
                a_rows=1, a_cols=8192, b_rows=8192, b_cols=8192, 
                sparsity=0.8, block_size=PE_SIZE)
    simulator.run_simulation(max_cycles=10000)
