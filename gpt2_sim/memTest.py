...
"""
内存模块测试
可以用 uv run gpt2_sim/memTest.py 运行
"""
...
import unittest
import torch
from gpt2_sim.memory_sim import _HardwareSimulator, SRAMOverflowError, DDROverflowError
...
class TestHardwareSimulator(unittest.TestCase):
    ...
    def tearDown(self):
        for log in self.sim.access_log:
            print(log)

    def test_sram_lru_eviction(self):
        """测试SRAM的LRU淘汰策略"""
        # 1. 写入两个半满数据块
        data_a = torch.randn(8, 16, dtype=torch.float32)  # 512 bytes
        data_b = torch.randn(8, 16, dtype=torch.float32)  # 512 bytes
    ...
    def test_ddr_load_and_access(self):
        """测试直接加载到DDR和从DDR访问数据"""
        # 1. 直接加载数据到DDR
        data_a = torch.randn(16, 16, dtype=torch.float32)  # 1024 bytes
    ...
    def test_ddr_eviction_when_full(self):
        ...
        with self.assertRaises(DDROverflowError):
            self.sim.ddr_load("overflow_test", torch.randn(
                8, 16), evict_if_full=False)
    ...
    def test_cycles_accounting(self):
        ...
        expected_delay = self.sim.ddr_read_delay + \
            self.sim.ddr_to_sram_delay + self.sim.read_delay
