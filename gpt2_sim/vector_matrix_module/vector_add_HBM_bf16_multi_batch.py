...
logger = logging.getLogger("VectorAddSimMultiBatch")
...
class VectorAddSimulatorWithHBMMultiBatch:
    ...
    def run_simulation(self, A_vectors_bf16, B_vectors_bf16, elements_per_block=2048):
        ...
        logger.info(f"多batch加法: A({batch_size}, {vector_size}) + B({batch_size}, {vector_size})")
        ...
        for block_idx in tqdm(range(num_blocks), desc="多batch加法 处理HBM块"):
        ...
        logger.info(f"多batch加法模拟完成. 总周期: {self.clock}, 总任务数: {total_tasks_generated}")
        return {
...
def main():
    ...
    logger.info("="*60)
    logger.info("测试HBM模式多batch向量加法模拟器")
    test_cases = [
    ...
    for test_case in test_cases:
        ...
        if batch_size > num_perows:
            ...
            continue
        logger.info(f"\n--- {desc} ---")
        A_vectors_fp32 = np.random.randn(batch_size, vector_size).astype(np.float32) * 0.1
        B_vectors_fp32 = np.random.randn(batch_size, vector_size).astype(np.float32) * 0.1
        ...
        simulator = VectorAddSimulatorWithHBMMultiBatch(
