...
from .bf16_sim import BF16AddPipeline, FP32toBF16Pipeline
    ...
    def clock_cycle(self):
        ...
        if self.stage2_input:
            ...
            if add_result["valid_output"] and self.add_pipeline.outputs:
                self.perow_result_vector[col_idx] = self.add_pipeline.outputs.pop(0)  # 直接使用BF16整数结果
...
class ProcessingElementRow:
    ...
    def __init__(self, perow_id, num_pes=128, vector_size=4096):
        ...
        # self.result_vector = [0] * vector_size  # 使用BF16格式存储结果
    ...
    def reset(self):
        # self.result_vector = [0] * self.vector_size  # 使用BF16格式存储结果
...
class VectorAddSimulatorWithHBM:
    ...
    def __init__(self, num_perows=32, pes_per_row=128, vector_size=4096):
        ...
        self.clock = 0
        # self.final_result_vector = [0] * vector_size  # 使用BF16格式存储结果
        self.final_result_vector = [fp32_to_bf16(0.0) for _ in range(vector_size)]
    ...
    def reset(self):
        ...
        for perow in self.perows:
            perow.reset()
        # self.final_result_vector = [0] * self.vector_size  # 使用BF16格式存储结果
        self.final_result_vector = [fp32_to_bf16(0.0) for _ in range(self.vector_size)] 
...
def main():
    ...
    logger.info("生成测试数据...")
    # 生成FP32数据并转换为BF16
    A_vector_fp32 = np.random.randn(vector_size).astype(np.float32) * 0.1
    B_vector_fp32 = np.random.randn(vector_size).astype(np.float32) * 0.1
